...
export const TimePicker: React.FC<TimePickerProps> = ({ format = 'H:i:s', value, min, max, onChange }) => {
    /**
     * ToDo: доделать или выпилить формат
     */
    const isHours = format.match(/H/g);
    const isMinutes = format.match(/i/g);
    const isSeconds = format.match(/s/g);
